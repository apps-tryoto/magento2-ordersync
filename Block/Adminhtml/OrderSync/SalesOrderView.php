...
class SalesOrderView extends \Magento\Backend\Block\Template
{
	...
	public function viewOrderSyncJobs()
	{
		$order_id = $this->getRequest()->getParam('order_id');

		if ($order_id < 1) 
		{
			return __('Order id is empty...');
		} // if sonu
		
		$jobs = $this->_orderSyncResource->getCollection()->addFieldToSelect('*');
        $jobs->addFieldToFilter('order_id', array('eq' => $order_id));
        $jobs->setOrder('job_id', 'asc');
